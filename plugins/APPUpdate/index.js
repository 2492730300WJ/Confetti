...
const getDownload = function(data) {
	...
	dtask = plus.downloader.createDownload(data.wgtUrl, {}, function(download, status) {
		if (status == 200) {
			popupObj.change({
				progressValue: 100,
				progressTip: "正在安装文件...",
...
function updatePopup(data, callback) {
	...
	// 描述的列表
	const descriptionList = drawtext(data.tips ? data.tips : '有新版本更新啦~，点击确定按钮立即更新', viewContentWidth);
	// 弹窗容器高度
	let popupViewHeight = 80 + 20 + 20 + 90 + 10;
	let popupViewContentList = [{
			src: 'http://47.102.121.70/file/ic_ar.png',
			id: "logo",
			tag: "img",
			position: {
				top: "0px",
